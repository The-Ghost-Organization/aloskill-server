  ...
  const booksData = await executeDbOperation(async prisma => {
    return await prisma.$transaction(async tx => {
      ...
      bookBreakdown.sort((a, b) => {
        const count = (book: typeof a) => book.orderItem.reduce((sum, item) => sum + item.quantity, 0);
      ...
      const weekStart = Date.UTC(local.getUTCFullYear(), local.getUTCMonth(), local.getUTCDate() - weekday) - offset;
      const previousStart = weekStart - 7 * 86400000;
      const emptyWeek = () => ({ units: 0, sales: 0, revenue: 0, dailyUnits: Array<number>(7).fill(0), dailySales: Array<number>(7).fill(0), dailyRevenue: Array<number>(7).fill(0) });
      ...
      for (const book of bookBreakdown) {
        for (const item of book.orderItem) {
          ...
          if (!week || time > now.getTime()) {continue;}
  ...
  return await executeDbOperation(async prisma => {
    return await prisma.$transaction(async tx => {
      ...
      const author = await tx.bookAuthor.findFirst({
        ...
        select: {
          id: true, name: true, slug: true, bio: true, photoUrl: true,
          websiteUrl: true, instructorProfileId: true, isActive: true, createdAt: true,
          books: {
            where: { deletedAt: null },
            select: {
              id: true, title: true, status: true, coverImage: true,
              formats: true, stock: true, viewCount: true,
              orderItem: {
      ...
      const books = author.books.map(book => {
        ...
        return {
          ...
          coverImage: book.coverImage, formats: book.formats, stock: book.stock,
          viewCount: book.viewCount, unitsSold, sales, reviewCount, rating,
        };
      });
...
const updateBookAuthor = async (req: Request) => {
  ...
  const input = req.body as {
    name?: string; bio?: string | null; photoUrl?: string | null;
    websiteUrl?: string | null; isActive?: boolean;
  };
  return await executeDbOperation(async prisma => {
    return await prisma.$transaction(async tx => {
      const admin = await getAuthorizedAdmin(tx, req.user.email);
      ...
      await tx.auditLog.create({
        data: {
          userId: admin.id,
          action: 'BOOK_AUTHOR_UPDATED', entityType: 'BOOK_AUTHOR', entityId: authorId,
