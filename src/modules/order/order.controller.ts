...
const verifyEPSPayment = catchAsync(async (req, res): Promise<void> => {
  const result = await orderService.verifyEPSPayment(req);
  ResponseHandler.ok(res, 'EPS payment status retrieved successfully.', result);
});
